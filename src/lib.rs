...
use telera_layout::{LayoutEngine, TextConfig, Vec2};
...
pub trait App<UserEvents, ImageElementData: Debug, CustomElementData: Debug, CustomLayoutSettings>{
    ...
    fn update(&self, layout: &mut LayoutEngine<ImageElementData, CustomElementData, CustomLayoutSettings>) -> Vec<RenderCommand::<ImageElementData, CustomElementData, CustomLayoutSettings>>;
...
struct Application<UserApp: App<UserEvents,(),(),()>, UserEvents: FromStr+Clone+PartialEq, UserPages: Default>{
    ...
    pub ui_renderer: Rc<RefCell<UIRenderer>>,
    ...
    where <UserEvents as FromStr>::Err: Debug
{
    pub fn new(app_events: EventLoopProxy<InternalEvents>, user_application: UserApp, watcher: ReadDirectoryChangesWatcher) -> Self {
        ...
        let ui_renderer = Rc::<RefCell<UIRenderer>>::new(RefCell::new(UIRenderer::new(
            &ctx.device, 
            &ctx.queue,
        )));
        ...
        let mut ui_layout = LayoutEngine::<(), (), ()>::new((1.0, 1.0));
        ui_layout.set_text_measurement(ui_renderer.clone(), measure_text);
